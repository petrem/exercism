...
import Counter (allCountsByFrequency, count)
...
yacht FullHouse dice =
  ...
  where
    isFullHouse = map snd (allCountsByFrequency dice) == [2, 3]
...
computeOnesToSixes category dice =
  (*) <$> id <*> flip count dice $ fromEnum category + 1

computeNofAKind :: Int -> Dice -> Score
computeNofAKind n dice = score_for_n $ last . allCountsByFrequency $ dice
  where
    score_for_n (which, how_many)
      | how_many >= n = which
      | otherwise = 0
