...
def _exchangeable_bills_and_remainder(
    budget: float, exchange_rate: float, spread: int, denomination: int
) -> Tuple[int, float]:
    rate = exchange_rate * (100 + spread) / 100
    bills, remainder = divmod(budget / rate, denomination)
    return int(bills), remainder
...
def exchangeable_value(
    ...
    bills, _ = _exchangeable_bills_and_remainder(
...
def non_exchangeable_value(
    ...
    _, remainder = _exchangeable_bills_and_remainder(
        budget, exchange_rate, spread, denomination
    )
