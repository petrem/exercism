...
#if _DEBUG_LEVEL > 0
#include <stdio.h>
#define DEBUG_PRINT(level, ...) if (_DEBUG_LEVEL >= (level)) printf(__VA_ARGS__)
#else
...
triplets_t *triplets_with_sum(triplet_int_t sum) {
  DEBUG_PRINT(1, "Triplets with sum %d\n", sum);
...
*/
static bool visit(fibbox_t primitive_box, void *payload) {
  DEBUG_PRINT(2, "Visiting (%d, %d, %d, %d) with perimeter %d\n",
  ...
  if (fibbox_perimeter(primitive_box) <= args->cutoff) {
    ...
    while (perimeter <= args->cutoff) {
      DEBUG_PRINT(2, "Multiple %d: (%d, %d, %d) with perimeter %d\n",
...
static op_triplets_t *add_result(op_triplet_t triplet,
  ...
  DEBUG_PRINT(1, "Adding result #%zu (%d, %d, %d)\n",
              results->count + 1, triplet.a, triplet.b, triplet.c);

  if (results->count >= *max_results) {
    DEBUG_PRINT(1, "Reallocating results array from %zu to %zu\n",
